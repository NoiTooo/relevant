...
class UserDetail(OnlyYouMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        pk = self.kwargs['pk']
        # お気に入りユーザーを表示
        specific = User.objects.get(pk=pk)
        r_client = specific.relevant.client.all()
        ctx['r_client'] = r_client
        r_adviser = specific.relevant.adviser.all()
        ctx['r_adviser'] = r_adviser
        # お気に入りユーザーをカウントする
        ctx['count_client'] = len(r_client)
        ctx['count_adviser'] = len(r_adviser)
        return ctx
